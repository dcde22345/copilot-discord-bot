...
export class SessionRuntimeManager {
    ...
    permissionPromptByMessageId = new Map();
    activeTurnStatus = new Map();
    lastRuntimeStartFailureAt = new Map();
    embeddingService;
    ...
    async ensureSessionRecord(thread) {
    ...
    buildRagContext(relatedConversations) {
    ...
    async sendUserMessage(thread, record, userText) {
        ...
        try {
            const statusMessage = await thread.send(buildStatusMessage({ state: "Working", repoDisplay, model, action: "Searching history...", elapsedSeconds: 0 }));
        ...
        let outcome = CopilotTurnOutcome.failed("unknown");
        try {
            const runtime = await this.getOrCreateRuntime(thread, record, ragContext);
            outcome = await runtime.sendUserMessage(userText);
        }
        catch (err) {
            await this.tryUpdateTurnStatus(BigInt(thread.id), { state: "Failed", action: err.message, force: true });
            outcome = CopilotTurnOutcome.failed(err.message);
        }
    ...
    tryResolvePermission(threadId, approve, toolCallId) {
    ...
    tryResolvePermissionByPromptMessageId(promptMessageId, approve) {
    ...
    buildPermissionHandler(thread, cfg) {
        return async (request) => {
            ...
            const perThread = this.pendingPermissions.get(threadId) ?? new Map();
            ...
            try {
                const promptMessage = await thread.send(box("Permission required", [{ key: "kind", value: kind }, { key: "detail", value: detail }], { footer: "React thumbsup / thumbsdown", icon: "pause" }));
    ...
    async getOrCreateRuntime(thread, record, ragContext = "") {
    ...
    async createRuntime(thread, record, ragContext = "") {
        ...
        const onConversationComplete = async (userQuestion, assistantAnswer) => {
            try {
                const embedding = await this.embeddingService.getCombinedEmbedding(userQuestion, assistantAnswer);
                if (embedding) {
                    const embeddingBuffer = Buffer.from(embedding.buffer);
                    this.store.saveConversationUnit(record.threadId.toString(), userQuestion, assistantAnswer, embeddingBuffer);
        ...
        const runtime = new CopilotRuntime(this.logger, record, cfg, cwd, this.config.COPILOT_CLI_PATH, this.config.TURN_TIMEOUT_SECONDS * 1000, sendToDiscord, sendProgress, this.buildPermissionHandler(thread, cfg), onConversationComplete, ragContext);
        if (ragContext) {
            try {
...
function looksLikeReadOnly(kindLower) {
    if (!kindLower)
        return false;
...
function buildPermissionDetail(request) {
    const extra = request;
    ...
    if (command && typeof command === "string")
        return `command: ${trimShort(command, 160)}`;
    if (filePath && typeof filePath === "string")
        return `path: ${trimShort(filePath, 180)}`;
    if (url && typeof url === "string")
        return `url: ${trimShort(url, 180)}`;
    return `kind: ${request.kind}`;
}
//# sourceMappingURL=SessionRuntimeManager.js.map