...
export class DiscordCommandRouter {
    ...
    client;
    repoPickers = new Map();
    constructor(logger, config, appConfig, store, runtimeManager, client) {
        this.logger = logger;
        this.config = config;
    ...
    async handleMessage(message) {
        if (message.author.bot)
            return;
        if (this.config.OWNER_DISCORD_USER_ID !== 0n &&
        ...
        if (message.channel.isThread()) {
            ...
            if (approval !== null) {
                const resolved = this.runtimeManager.tryResolvePermission(BigInt(thread.id), approval.approve, approval.toolCallId);
                if (resolved) {
    ...
    async handleReaction(reaction, user) {
        ...
        if (picker) {
            ...
            if (idx < 0 || idx >= picker.repoFullPaths.length)
                return;
            ...
            if (!isWithinRoot(pickedPath, this.config.REPOS_ROOT)) {
                await thread.send(box("Repo not allowed", [{ key: "path", value: pickedPath }, { key: "ReposRoot", value: this.config.REPOS_ROOT }], { footer: "Pick a repo under ReposRoot.", icon: "shield" }));
                return;
            }
            ...
            await thread.send(box("Repo selected", [{ key: "repo", value: pickedPath }], {
    ...
    // Private helpers
    // -------------------------------------------------------------------------
    async createThreadAndRespond(message, userPrompt) {
    ...
    async sendRepoPickerToThread(thread, deferredPrompt) {
        if (!existsSync(this.config.REPOS_ROOT)) {
            await thread.send(box("Repos root missing", [{ key: "ReposRoot", value: this.config.REPOS_ROOT }], {
        ...
        if (repoPaths.length === 0) {
            await thread.send(box("No repos found", [{ key: "ReposRoot", value: this.config.REPOS_ROOT }], {
        ...
        const pickerMessage = await thread.send("**Select repo**\n" +
            "```text\n" +
            lines.join("\n") +
            "\n```\n" +
            "React 1️⃣..🔟 to choose\n" +
    ...
    async handleCommand(message, content) {
        ...
        if (cmdLower === "" || cmdLower === "help") {
            await replyChunked(message.channel, box("Copilot bot", [
        ...
        if (cmdLower === "repos") {
            ...
            const dirs = readdirSync(this.config.REPOS_ROOT, { withFileTypes: true })
                ...
                .sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()))
                .slice(0, 50);
            await replyChunked(message.channel, "**Repos**\n```text\n" + dirs.map((d) => `- ${d}`).join("\n") + "\n```");
            return;
        }
        if (cmdLower === "init") {
            if (!message.channel.isThread()) {
        ...
        if (cmdLower === "repo") {
            ...
            await thread.send(box("Repo selected", [{ key: "repo", value: full }], {
        ...
        if (cmdLower === "model") {
            ...
            if (!rest.trim()) {
                ...
                await thread.send(box("Model", [{ key: "current", value: current }], {
        ...
        if (cmdLower === "tools") {
            ...
            if (sub.toLowerCase() === "allow") {
                ...
                await thread.send(box("Usage", [{ key: "allow", value: "cp tools allow view,edit" }, { key: "exclude", value: "cp tools exclude edit" }], { icon: "bulb" }));
                return;
            }
            this.store.upsert({ ...record, configJson: this.store.writeConfig(cfg), updatedUtc: new Date() });
            this.runtimeManager.invalidateRuntime(BigInt(thread.id));
        ...
        if (cmdLower === "config") {
            ...
            if (sub.toLowerCase() === "set") {
                try {
                    ...
                    await thread.send(`Config JSON invalid: ${err.message}`);
                }
                return;
            }
        ...
        if (cmdLower === "approve") {
            ...
            if (mode !== "always" && mode !== "ask") {
                await thread.send(box("Approval policy", [{ key: "ask", value: "cp approve ask" }, { key: "always", value: "cp approve always" }], { footer: "`always` is risky — use only if you trust the repo + tools.", icon: "shield" }));
                return;
            }
            let record = this.store.getByThreadId(BigInt(thread.id));
            record ??= await this.runtimeManager.ensureSessionRecord(thread);
            ...
            this.store.upsert({ ...record, configJson: this.store.writeConfig(cfg), updatedUtc: new Date() });
            await thread.send(box("Approval policy", [{ key: "mode", value: cfg.autoApprovePermissions ? "always" : "ask" }], {
...
function trimCommandPrefix(content) {
    if (content.toLowerCase().startsWith("!cp"))
...
function tryParseApproval(content) {
...
async function replyChunked(channel, text) {
    if (!("send" in channel))
        return;
...
//# sourceMappingURL=DiscordCommandRouter.js.map