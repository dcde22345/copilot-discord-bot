...
export class SqliteSessionStore {
    ...
    upsert(record) {
        this.db
            .prepare(`INSERT INTO sessions (thread_id, guild_id, parent_channel_id, copilot_session_id, repo_path, config_json, created_utc, updated_utc)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(thread_id) DO UPDATE SET
           repo_path = excluded.repo_path,
           config_json = excluded.config_json,
           updated_utc = excluded.updated_utc`)
            .run(record.threadId.toString(), record.guildId.toString(), record.parentChannelId.toString(), record.copilotSessionId, record.repoPath, record.configJson, record.createdUtc.toISOString(), record.updatedUtc.toISOString());
    }
    list() {
        const rows = this.db
    ...
    saveConversationUnit(threadId, userQuestion, assistantAnswer, embeddingBuffer) {
        const now = new Date().toISOString();
        const insertStmt = this.db.prepare(`INSERT INTO conversation_units (thread_id, user_question, assistant_answer, created_utc, embedding)
       VALUES (?, ?, ?, ?, ?)`);
        const result = insertStmt.run(threadId, userQuestion, assistantAnswer, now, embeddingBuffer);
        const rowid = result.lastInsertRowid;
        if (embeddingBuffer !== null) {
            const vecInsertStmt = this.db.prepare(`INSERT INTO conversation_vectors (rowid, embedding)
         VALUES (?, ?)`);
            vecInsertStmt.run(rowid, embeddingBuffer);
        }
        return rowid;
    }
    searchTopKConversations(queryEmbedding, k) {
        const sql = `
      SELECT
        cu.id,
        cu.thread_id,
    ...
    `;
        ...
        const rows = stmt.all(new Uint8Array(queryEmbedding.buffer), k);
        return rows.map(row => ({
            id: row.id,
...
//# sourceMappingURL=SqliteSessionStore.js.map